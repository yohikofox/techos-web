...
export interface ICookieManager {
  ...
  update(key: string, value: string, options?: CookieOption): void;
  invalidate(key: string): void;
  get(key: string): string | undefined;
  add(key: string, value: string, options?: CookieOption): void;
  render(): string;
...
export default class CookieManager implements ICookieManager {
  ...
  public updateKey(key: string, newKey: string): void {
    ...
    this.updateCookieOptions(parsedNewKey.key, {
  ...
  public update(key: string, value: string, options?: CookieOption): void {
    const parsedKey = CookieManager.parseKey(key);

    if (this._store[parsedKey.key] === undefined) return;

    this._store[parsedKey.key].value = value;
    this.updateCookieOptions(parsedKey.key, {
      ...options,
  ...
  private updateCookieOptions(key: string, options: CookieOption) {
    const parsedKey = CookieManager.parseKey(key);
  ...
  public add(key: string, value: string, options?: CookieOption): void {
    const parsedKey = CookieManager.parseKey(key);
    this._store[parsedKey.key] = {
      value: value,
      option: {
  ...
  public render(): string {
    return Object.keys(this._store).length > 0
      ? CookieManager.stringify(this._store)
      : "";
  }
  ...
  public static stringify(cookies: CookieCollection): string {
    return Object.entries(cookies)
      .map(([key, value]) => {
        if (value.option.secure !== undefined)
          return `${SECURE_PREFIX}${key}=${value.value}`;
        if (value.option.isHost !== undefined)
