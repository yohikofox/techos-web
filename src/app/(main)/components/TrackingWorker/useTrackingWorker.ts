...
const useTrackingWorker = ({ data, initialData }: any): [number, Dispatch<number>] => {
  const [counter, setCounter] = useState(initialData || 0)
  useEffect(() => {
    const worker = new Worker(new URL('@/app/(main)/components/TrackingWorker/worker.ts', import.meta.url))
