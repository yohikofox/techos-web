...
export async function GET(request: NextRequest, params: any) {
  ...
  const response = await new NextAuthManager().getSignOutRedirectData(callbackUrl || '/', { headers: new Headers(request.headers) }, { isSecured: RequestHelper.isSecured(request) })
