...
export default function Component({ initialModel }: ProductProps) {
  ...
  const [category, _] = useState(model.category);
    ...
    <>
      <section className={styles.content}>
        ...
        <div className={styles.image}>
          <Image src={model.image} alt={model.title} fill />
        </div>
        <button className={styles.button} onClick={handleSameCategory}>
          Voir un autre cadeau similaire
        </button>
      </section>
    </>
  );
