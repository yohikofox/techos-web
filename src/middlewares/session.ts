...
const PATTERNS = ["/api/admin", "/admin"];
...
export default class SessionMiddleware extends Middleware {
  ...
  async run(
    ...
    console.log("🚀 ~ SessionMiddleware ~ pathName:", pathName);
    console.log(
      "🚀 ~ SessionMiddleware ~ process.env.NEXT_PUBLIC_FRONT_URL:",
      process.env.NEXT_PUBLIC_FRONT_URL
    );

    const response = await fetch(
      `${process.env.NEXT_PUBLIC_FRONT_URL}/api/middleware/session?callbackUrl=${pathName}`,
      {
        next: {
          revalidate: 0,
        },
    ...
    const { redirectUrl, cookies, isError } =
      (await response.json()) as RedirectData;
